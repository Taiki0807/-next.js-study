...
const TitleSection = ({ title, subheading, pill }: TitleSectionProps) => {
  return (
    <section className="flex flex-col gap-4 justify-center items-start md:items-center">
      ...
      {subheading ? (
        <>
          <h2 className="text-left text-3x sm:text-5xl sm:max-w-[750px] md:text-center font-semibold">
            {title}
          </h2>
        ...
        <h1 className="text-left text-4x sm:text-6xl sm:max-w-[850px] md:text-center font-semibold">
          {title}
        </h1>
      )}
    </section>
